...
export default class Auth extends EventEmitter {
  ...
  // The methods below are bound in the contructor with 'this
  constructor() {
    super();
  ...
  getProfile(cb) {
    ...
    // Makes a call to the /userinfo endpoint of the
    // auth0 domain and returns the user profile
  ...
  logout() {
    // Clear access token and ID token from the local storage
    localStorage.removeItem('acess_token');
    localStorage.removeItem('id_token');
    localStorage.removeItem('expires_at');
    this.userProfile = null;
    // navigat to the home route
    history.replace('/home');
  }

  isAuthenticated() {
    // Check whether the current time is past the
    // access token's epiry time
